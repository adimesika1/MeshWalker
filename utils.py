...
SEGMENTATION_COLORMAP = np.array(
  ((165, 242, 12), (165, 12, 89), (12, 89, 89), (165, 165, 89), (89, 242, 12), (226, 144, 221), (255, 196, 255), (242, 242, 165),
   (242, 165, 12), (232, 139, 127), (165, 160, 160), (165, 89, 242), (12, 12, 165), (12, 89, 165), (242, 242, 89), (165, 255, 255)),
  ...
  for pos in range(n_obj2show):
    ...
    if type(walk) is list:
      for i, w in enumerate(walk):
        ...
        p.add_mesh(walk_mesh, show_edges=True, line_width=line_width * 4, edge_color=colors[i%(len(colors))])
    ...
    if params.full_accuracy_test is not None:
      if params.network_task == 'semantic_segmentation':
        ...
        with open(params.logdir + '/log.txt', 'at') as f:
          f.write('Accuracy: ' + str(np.round(np.array(accuracy[1:]) * 100, 3)) + '%, Iter: ' + str(iterations.numpy()) + '\n')
        tf.summary.scalar('full_accuracy_test/class_mean_iou', accuracy[1], step=iterations)
        tf.summary.scalar('full_accuracy_test/instance_mean_iou', accuracy[2], step=iterations)
        tf.summary.scalar('full_accuracy_test/naive_accuracy', accuracy[3], step=iterations)

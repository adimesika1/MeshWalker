...
def train_val(params):
  ...
  for i in range(len(params.datasets2use['train'])):
    this_train_dataset, n_trn_items = dataset.tf_mesh_dataset(params, params.datasets2use['train'][i],
                                                              ...
                                                              min_dataset_size=128, data_augmentation=params.train_data_augmentation)
  ...
  if params.optimizer_type == 'adam':
    ...
    #optimizer = tfa.optimizers.AdamW(learning_rate=lr_schedule, weight_decay=1e-4, clipnorm=params.gradient_clip_th)
  ...
  if params.last_layer_actication is None:
    ...
    seg_loss = tf.keras.losses.SparseCategoricalCrossentropy() #SparseCategoricalFocalLoss(gamma=2) #
...
def get_params(job, job_part):
  ...
  if job == 'partnet':
    params = params_setting.partnet_params()

  if job == 'partnet_pn2':
    params = params_setting.partnet_pn_params()
...
if __name__ == '__main__':
  ...
  if len(sys.argv) <= 1:
    ...
    print('<job> can be one of the following: shrec11 / coseg / human_seg / cubes / modelnet40 / partnet / partnet_pn2')
